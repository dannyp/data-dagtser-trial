...
class DuckDBPartitionedParquetIOManager(PartitionedParquetIOManager):
    """Stores data in parquet files and creates duckDB views over them"""
    ...
    def load_input(self, context) -> str:
        check.invariant(not context.has_asset_partitions, "Can't load partitioned input")

        if context.dagster_type.typing.type == pd.DataFrame:
            con = self._connect_duckdb(context)
    ...
    def _table_path(self, context):
        context.log.info(f"asset_key.path: {context.asset_key.path}")
